...
def _build_text(genres: str, overview: str) -> str:
    """
    Gabung genre (3×) + overview sesuai keputusan desain:
    text = (genres_clean + ' ') * 3 + overview_clean
    """
...
def build_movie_index() -> tuple[TfidfVectorizer, list[int]]:
    ...
    df['text_combined'] = df.apply(
        lambda r: _build_text(r.get('genres_clean', ''), r.get('overview_clean', '')),
        axis=1
    )
    ...
    for idx, mid in enumerate(movie_ids):
        cur.execute("UPDATE films SET tfidf_vector_idx = ? WHERE movieId = ?", (idx, mid))
    conn.commit()
    conn.close()
...
def build_case_index(vectorizer: TfidfVectorizer) -> list[int]:
    ...
    for c in cases:
        ...
        # Case embedding: query + weighted genres + title
        text = query + ' ' + (genres + ' ') * 3 + title
